...
export default function CountrySelector({ selected, onSelect }: CountrySelectorProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return countriesWithMinWage.filter(
      (c) =>
        c.name.toLowerCase().includes(term) ||
        c.nameLocal.toLowerCase().includes(term) ||
  ...
  const filteredNoMin = useMemo(() => {
    ...
    return countriesWithoutMinWage.filter(
      (c) =>
        c.name.toLowerCase().includes(term) ||
        c.nameLocal.toLowerCase().includes(term) ||
        c.code.toLowerCase().includes(term)
    ...
    <div className="space-y-4">
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/40 focus:outline-none focus:border-amber-400/50 focus:ring-1 focus:ring-amber-400/30 transition-colors"
        />
        ...
        <AnimatePresence mode="popLayout">
          {filtered.map((country) => (
            <motion.button
              ...
              className={`flex flex-col items-center gap-1 p-3 rounded-xl border transition-colors cursor-pointer ${selected?.code === country.code ? "bg-amber-400/15 border-amber-400/50 shadow-lg shadow-amber-400/10" : "bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20"}`}
            >
              <span className="text-2xl">{country.flag}</span>
              <span className="text-xs font-medium text-white/90 text-center leading-tight">{country.name}</span>
              <span className="text-[10px] text-amber-400/80 font-mono">{country.minimumWage.grossMonthly.toLocaleString("pt-BR")} €</span>
            </motion.button>
          ))}
        </AnimatePresence>
      </div>
      ...
      {(filteredNoMin.length > 0) && (
        <div className="mt-4">
          <button
            onClick={() => setShowNoMinWage(!showNoMinWage)}
            className="text-sm text-white/50 hover:text-white/70 transition-colors flex items-center gap-2"
          >
            <svg className={`w-4 h-4 transition-transform ${showNoMinWage ? "rotate-90" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
            Países sem salário mínimo estatutário ({filteredNoMin.length})
          </button>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 mt-3">
                  {filteredNoMin.map((country) => (
                    ...
                    >
                      <span className="text-2xl">{country.flag}</span>
                      <span className="text-xs font-medium text-white/90 text-center leading-tight">{country.name}</span>
                      <span className="text-[10px] text-blue-400/80 italic">Acordo coletivo</span>
                    </motion.button>
                  ))}
                </div>
              </motion.div>
