...
export default function SalaryCalculator({ country }: Props) {
  ...
  }, [customSalary, hoursWorked, daysWorked, country.code]);
  ...
  const calc = useMemo(() => {
    ...
    const standardMonthlyHours = standardHours * weeksPerMonth;
    const standardMonthlyDays = daysPerWeek * weeksPerMonth;

    const hourly = baseSalary / standardMonthlyHours;
    const daily = baseSalary / standardMonthlyDays;
    ...
    if (inputHours !== null && inputHours > 0) {
      ...
      const overtimePay = overtimeHrs * hourly * 1.5; // standard 150% overtime
      hoursCalc = {
    ...
    return { hourly, daily, weekly, monthly, annual, totalDeductions, standardMonthlyHours: Math.round(standardMonthlyHours), standardMonthlyDays: Math.round(standardMonthlyDays), hoursCalc, daysCalc };
  }, [baseSalary, country, inputHours, inputDays]);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <input type="number" min="0" step="0.01" placeholder={baseSalary.toString()} value={customSalary} onChange={(e) => setCustomSalary(e.target.value)}
          ...
          <p className="text-xs text-white/30 mt-1">{t.minimum}: {fmt(country.minimumWage.grossMonthly)} €</p>
        </div>
        {/* Hours worked */}
        <div>
          <label className="block text-sm text-white/60 mb-1">{t.hoursWorked}</label>
          <input type="number" min="0" step="0.5" placeholder={calc ? calc.standardMonthlyHours.toString() : ""} value={hoursWorked} onChange={(e) => setHoursWorked(e.target.value)}
            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-blue-400/50 focus:ring-1 focus:ring-blue-400/30 font-mono" />
          <p className="text-xs text-white/30 mt-1">{t.workWeek}: {country.workWeek.standardHours}h (~{calc?.standardMonthlyHours}h/{t.month})</p>
        </div>
        {/* Days worked */}
        <div>
          <label className="block text-sm text-white/60 mb-1">{t.daysWorkedMonth}</label>
          <input type="number" min="0" step="1" placeholder={calc ? calc.standardMonthlyDays.toString() : ""} value={daysWorked} onChange={(e) => setDaysWorked(e.target.value)}
            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-blue-400/50 focus:ring-1 focus:ring-blue-400/30 font-mono" />
          <p className="text-xs text-white/30 mt-1">{country.workWeek.daysPerWeek} {t.days}/{t.week} (~{calc?.standardMonthlyDays} {t.days}/{t.month})</p>
        </div>
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {[
              ...
              { label: `${t.perYear} (${country.minimumWage.annualPayments}x)`, value: calc.annual, highlight: true },
            ].map((item) => (
              <motion.div key={item.label} whileHover={{ scale: 1.02 }} className={`p-4 rounded-xl border ${item.highlight ? "bg-blue-400/10 border-blue-400/30" : "bg-white/5 border-white/10"}`}>
          ...
          {calc.hoursCalc && (
            <div className="bg-gradient-to-r from-cyan-500/[0.08] to-transparent border border-cyan-400/20 rounded-2xl p-5">
              <p className="text-xs text-cyan-300/70 uppercase tracking-wider mb-3 font-medium">{t.results} — {calc.hoursCalc.totalHours}h {t.hoursWorked.toLowerCase().replace(/\(.*\)/, "").trim()}</p>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 text-sm text-white/40">
            <div className="flex flex-wrap gap-x-6 gap-y-1">
              <span>{t.workWeek}: <strong className="text-white/60">{country.workWeek.standardHours}h/{t.week}</strong></span>
              <span>{t.payments}: <strong className="text-white/60">{country.minimumWage.annualPayments}x/{t.perYear.toLowerCase()}</strong></span>
              <span>{t.definedBy}: <strong className="text-white/60">{country.minimumWage.definedPer === "hour" ? t.hour : country.minimumWage.definedPer === "week" ? t.week : t.month}</strong></span>
